...
class LidarObstacleDetector(Node):
    ...
    def process_roi(self, points):
        ...
        t_parking_indices = (
            (points[:, 0] > self.t_parking_roi_x_min) & (points[:, 0] < self.t_parking_roi_x_max) &
            (points[:, 1] > -self.t_parking_roi_y_width / 2) & (points[:, 1] < self.t_parking_roi_y_width / 2)
        )
        t_parking_points = points[t_parking_indices]

        if len(center_points) > 0 or len(slow_points) > 0 or len(side_points) > 0 or len(t_parking_points) > 0:
            """
            ...
            if len(t_parking_points) > 30:
                self.lidar_rddf_select_msg.data = True
            else:
                self.lidar_rddf_select_msg.data = False
        ...
        self.lidar_rddf_select_publisher.publish(self.lidar_rddf_select_msg)
