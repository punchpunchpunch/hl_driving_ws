...
class CommCar(Node):
    ...
    def control_loop(self):
        if self.flag == 1: # GPS FLAG
            steer = self.gps_steer
            speed = self.gps_speed
            mode = 'gps'
        elif self.side_ok: #lidar_side
        ...
        if self.flag == 1: # GPS FLAG
            pass
        elif self.center_ok: #lidar_center
            speed = self.center_speed
            mode = mode + ' center'
        elif self.slow_ok: #lidar_slow
            speed = self.slow_speed
            mode = mode + ' slow'

        hst = self.steer_to_hst(steer)
        self.send_frame(speed, hst)
