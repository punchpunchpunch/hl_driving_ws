...
class WaypointSaver(Node):
    ...
    def __init__(self):
        ...
        self.ofs.write('utm_x,utm_y,flag,rddf_num\n')
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        self.get_logger().info(
            f'UTM: {utm_x:.6f}, {utm_y:.6f}, flag={self.flag}, rddf_num={self.rddf_num}'
        )

        self.ofs.write(
            f'{utm_x:.6f},{utm_y:.6f},{self.flag},{self.rddf_num}\n'
        )
        self.ofs.flush()
    ...
    def set_rddf_num(self, rddf_num):
        self.rddf_num = rddf_num
...
class UI(QWidget):
    ...
    def __init__(self, node):
        ...
        self.rddf_label = QLabel("Current RDDF Num: 0")
        ...
        layout.addWidget(self.rddf_label)
        ...
        # rddf_num 버튼
        for i in range(2):
            btn = QPushButton(f"RDDF {i}")
            btn.setMinimumHeight(50)
            btn.clicked.connect(lambda _, num=i: node.set_rddf_num(num))
            layout.addWidget(btn)
    ...
    def update_status(self):
        ...
        self.rddf_label.setText(f"Current RDDF Num: {self.node.rddf_num}")
